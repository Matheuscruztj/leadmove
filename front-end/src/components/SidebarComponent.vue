<template>
  <div :class="['flex flex-col h-screen bg-purple-700 text-white transition-all duration-300', isCollapsed ? 'w-20' : 'w-52 md:w-48']">
    <div class="flex items-center justify-between h-16 bg-purple-800 px-4">
    ...
    <nav class="flex-1 px-4 py-2">
      <ul>
        <li class="py-2" v-for="item in menuItems" :key="item.name">
          <a :href=item.href class="px-4 py-2 rounded flex items-center hover:bg-purple-600 transition-all duration-300 transform hover:-translate-y-1" :class="{'opacity-0': isCollapsed, 'opacity-100': !isCollapsed}">
