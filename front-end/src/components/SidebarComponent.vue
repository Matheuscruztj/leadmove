<template>
  <div :class="['flex flex-col h-screen bg-gray-900 text-white transition-all duration-300', isCollapsed ? 'w-20' : 'w-52 md:w-48']">
    ...
    <nav class="flex-1 px-4 py-2">
      <ul>
        <li class="py-2" v-for="item in menuItems" :key="item.name">
          <a :href=item.href class="px-4 py-2 rounded flex items-center hover:bg-gray-800 transition-all duration-300 transform hover:-translate-y-1" :class="{'opacity-0': isCollapsed, 'opacity-100': !isCollapsed}">
...
export default defineComponent({
  ...
  setup() {
    ...
    const menuItems = [
      { name: t('menu.dashboard'), icon: HomeIcon, href: '/' },
      { name: t('menu.post'), icon: PencilIcon, href: '/post' },
      { name: t('menu.user'), icon: UserIcon, href: '/user' },
      { name: t('menu.setting'), icon: CogIcon, href: '/setting' },
    ];
